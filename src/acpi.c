...
inline int read_pwr_files(struct pwr_sup *info, char *ac, char **batt, int limit)
{
     ...
     int jdx = 0;
     do {
	  /* FIXME prevents a memory leak with `tmp',
	   * moving it even just a couple of lines down seems to break
	   * every thing */
...
inline int find_pwr_files(glob_t globuf, char *ac, char **batt, int limit)
{
     int result = 0;
     FILE *fp;
     char *path;
     ...
     do {
	  ...
	  if (strncmp(tmp, ZB_ACPI_BATTYPE, 3) == 0 && limit-- > 0) {
	       fclose(fp);
	       path = concat(dirname(globuf.gl_pathv[idx]), "/capacity", NULL);
...
int pwr_info(struct pwr_sup *info, int btlimit)
{
     ...
     char *ac = alloca(ZB_ACPI_PATH_SIZE+1);
     char **batt = alloca((ZB_ACPI_PATH_SIZE+1)*btlimit);

     err = find_pwr_files(globuf, ac, batt, btlimit);
     globfree(&globuf);
